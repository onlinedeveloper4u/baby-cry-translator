...
import { getBaby, insertBaby, updateBaby, uploadBabyAvatar } from '../../src/api/babies';
  ...
  React.useEffect(() => {
    ...
    async function loadExisting() {
      ...
      try {
        ...
        // Fallback to local if not found in Supabase
    ...
    try {
      if (user?.id) {
        ...
        if (avatarUri && !/^https?:\/\//i.test(avatarUri)) {
          try {
            console.log('[Edit Baby] uploading avatar from', avatarUri);
            avatarUrlToSave = await uploadBabyAvatar(avatarUri, user.id);
            console.log('[Edit Baby] uploaded avatar path', avatarUrlToSave);
          } catch (e: any) {
        ...
        if (editingId) {
          await updateBaby(editingId, { name: trimmed, birthDate: birthDate || null, gender, notes: null, avatarUrl: avatarUrlToSave });
        } else {
          await insertBaby({ userId: user.id, name: trimmed, birthDate: birthDate || null, gender, notes: null, avatarUrl: avatarUrlToSave ?? null });
        }
      } else {
  ...
  return (
    <SafeAreaView className="flex-1 bg-rose-50">
      ...
      <View className="flex-row items-center justify-between px-6 pt-4 pb-2">
        ...
        <Text className="text-xl font-bold text-neutral-900">Edit Baby Profile</Text>
        <View style={{ width: 24 }} />
      </View>
